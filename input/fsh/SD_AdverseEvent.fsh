...
// 2. Participant (actors involved and their role in the AE)
// It would be straightforward to add these back in if desired.
...
// ------Grade-----
* insert NotUsed(severity)  // severity is not used because the required value set doesn't go with CTCAE grade
* extension contains 
    CTCAEGrade named grade 1..1 MS and
    AdverseEventResolvedDate named resolvedDate 0..1 MS and
    AdverseEventOutcome named seriousnessOutcome 0..1 MS
// ------Seriousness------
* seriousness 0..1 MS
...
Extension: AdverseEventOutcome // named seriousnessOutcome 0..1 MS
...
* value[x] only CodeableConcept
* value[x] from AdverseEventOutcomeVS (required)
...
* ^experimental = true
* NCIT#C41356 "Definite Adverse Event Attribution to Product or Procedure"
* NCIT#C41357 "Probable Adverse Event Relationship to Product or Procedure"
* NCIT#C41359 "Possible Adverse Event Attribution to Product or Procedure"
